# -*- coding: utf-8 -*-
"""
大创项目：基于 ImageBind 统一隐空间与多任务路由网络的多模态猫狗情绪识别
正式训练脚本（学术级）

技术栈（与 train/3_dinov3_convnext_finetune.py 对齐的去过补偿版）：
  - 分层采样：保证四源数据各类别比例在 train/val/test 中一致
  - 只保留 WeightedRandomSampler + FocalLoss(γ)：Batch 层已逆频均衡，Loss 中不再叠加逆频率 class_weights
  - 四任务损失等权（1.0）：不再 sqrt(N_max/N_task) 放大稀疏任务，避免与采样器双重放大猫/少样本分支
  - 物种辅助损失系数 0.3（与 dinov3 脚本一致）
  - 少数侧特征噪声：图像猫少强化猫图、音频狗少强化狗音频（与数据分布一致，避免全局「猫」在音频上过补偿）
  - 特征空间 Mixup：对离线 1024 维特征做线性插值增强，提升泛化能力
  - 残差 MLP + LayerNorm：抵御梯度消失，加速收敛
  - Warmup + CosineAnnealingLR：前 5 轮线性升温，后续余弦退火
  - 完整评估：per-class Precision/Recall/F1 + 混淆矩阵热力图
  - 早停与最优权重：balanced_macro_F1（四任务 macro-F1 平均），不用验证准确率
  - 四任务独立宏 F1 训练曲线可视化
  - CUDA 确定性模式 + 环境信息完整记录
...
输入：  data/features_npy/*.npy（由 extract_features.py 预先提取）
输出：
  moxing/MultiBranchMLP_{时间戳}.pkl
  figure/train_curve_{时间戳}.png
  figure/confusion_split_image_{时间戳}.png、figure/confusion_split_audio_{时间戳}.png
  figure/interpret_*_{时间戳}.png（各类别 P/R/F1、ROC、置信度、Top 混淆，见 imagebindtrain.mlp_interpretability）
  txt/train_log_{时间戳}.txt
"""
...
log_path = TXT_DIR / f"train_log_{TS}.txt"
...
WEIGHT_DECAY  = 2e-4
HIDDEN_DIM    = 512
PATIENCE      = 20           # 早停耐心（增大以给 k 折充分探索空间）
WARMUP_EPOCHS = 5
TRAIN_RATIO   = 0.8
VAL_RATIO     = 0.1
MIXUP_ALPHA   = 0.3          # Mixup Beta 分布参数，0 则关闭
FOCAL_GAMMA   = 2.0          # Focal Loss 聚焦参数
LABEL_SMOOTH  = 0.05
NOISE_STD     = 0.05         # 训练时隐层基础高斯噪声
# 少数模态侧隐层噪声倍数（与 1_languagebind / dinov3 思路一致）
...
# 测试时增强
TTA_STEPS     = 10           # 每个样本推理次数
TTA_NOISE_STD = 0.03         # TTA 输入特征扰动噪声强度

# 各任务分类头 Dropout（数据越少 → 正则化越强）
TASK_DROPOUTS = {
    "dog_img":   0.25,   # 图像狗样本多
    "cat_img":   0.40,   # 图像猫样本少 → 更强正则
    "dog_audio": 0.32,   # 音频狗少 → 提高 Dropout
    "cat_audio": 0.26,   # 音频猫多
}
...
class FocalLoss(nn.Module):
    """
    Focal Loss (Lin et al., 2017) 自适应降低易分类样本权重，
    聚焦于困难样本，对长尾分布和类别不平衡场景效果显著。
    支持 label smoothing。
    """
...
class MultimodalFeatureDataset(Dataset):
    """读取 ImageBind 离线 .npy 特征，整合四个数据源并构建联合标签。"""
    ...
    def __init__(self, feat_dir: Path):
        ...
        for task_id, (task_name, meta) in enumerate(TASK_META.items()):
            ...
            logger.info(f"  加载 {task_name}: {N} 条，{n_cls} 类")
        ...
        # 联合标签：(task_id, label) → 唯一整数，用于分层采样
        self.stratify_labels = (self.task_ids * 100 + self.labels).numpy()

        logger.info(f"  数据集合并完毕：共 {len(self.features)} 条样本")
...
def stratified_split(dataset: MultimodalFeatureDataset, train_ratio, val_ratio, seed):
    """
    基于 (task_id, class_label) 联合标签做分层采样，
    确保 train/val/test 各集中每个任务的每个类别比例与全集一致。
    """
...
def make_weighted_sampler(dataset: MultimodalFeatureDataset, indices: np.ndarray) -> WeightedRandomSampler:
    """
    按 (task_id, class_label) 的逆频率构造采样权重，
    使训练时每个类别被采样的期望次数大致相同。
    """
...
def mixup_batch(batch: dict, alpha: float) -> dict:
    """
    特征空间 Mixup (Zhang et al., 2018)：
    只在同一 task_id 内部做线性插值，避免不同任务/不同类别数的标签混乱。
    """
    if alpha <= 0:
        return batch
...
class ResidualBlock(nn.Module):
    """带残差连接的 MLP 块，使用 LayerNorm 替代 BatchNorm 以增强小批量稳定性。"""
...
class TaskHead(nn.Module):
    """两层任务分类头，为多类别任务（如 cat_img 7类、cat_audio 10类）提供更强建模能力。"""
...
class MultiBranchRoutingMLP(nn.Module):
    ...
    结构：
      Input(1024) → L2Norm → 降维(hidden) → GaussNoise(train, 少数侧更强) → ResBlock × 2 → TaskHead × 4
    物种分类作为辅助任务，引导共享层学习物种判别特征。
    """

    def __init__(self, input_dim: int = 1024, hidden_dim: int = HIDDEN_DIM):
        super().__init__()
...
def get_warmup_cosine_scheduler(optimizer, warmup_epochs: int, total_epochs: int, min_lr_ratio: float = 0.01):
    """前 warmup_epochs 轮线性升温，之后余弦退火到 min_lr_ratio * base_lr。"""
...
# ============================================================
# 损失计算（与 dinov3：采样器均衡后 Focal 不再带 class weight，任务等权）
# ============================================================
...
def compute_loss(
    ...
    """
    计算总损失并返回各分支 loss 用于日志记录。
    支持 Mixup + sqrt 逆频率任务损失加权（样本少的任务权重更高）。
    """
    ...
    for task in TASK_HEADS:
        ...
        if mixup_active and perm is not None and same_task is not None:
            # perm_labels[i] 来自全局打乱后的位置，可能属于其他任务，标签值对当前任务越界。
            # 因此只对 same_task=True（同任务内部的 Mixup 对）计算混合 loss；
            # same_task=False 的样本仍使用原始标签计算 loss。
            task_same = same_task[mask]                    # (N_task,) bool
            perm_labels_global = labels[perm][mask]        # (N_task,) 全局打乱后的标签
            n_cls = logits_t.shape[1]

            # 将越界的 perm 标签替换为 0（这些位置 same_task=False，后续不会被使用）
            safe_perm_labels = perm_labels_global.clone()
            safe_perm_labels[~task_same] = 0
            # 进一步裁剪，防止 same_task=True 但来自类别数不同子集的极端边角情况
            safe_perm_labels = safe_perm_labels.clamp(0, n_cls - 1)

            # per-sample focal loss（reduction="none"）
            ...
            # per-sample 选择：same_task → mixup 混合；否则只用原标签
        ...
        # 与 dinov3 一致：层均衡采样后任务间不再额外放大（等价 CAT_TASK_WEIGHT=1）
...
def predict_one_model_tta(
    ...
    """
    对单个模型执行测试时增强（TTA）推理。
    向输入特征注入 n_tta 次随机高斯噪声，对 softmax 概率取均值后返回。
    返回 {task: (avg_probs: ndarray, targets: ndarray)}。
    """
    model.eval()
...
def full_evaluation_ensemble(
    ...
    """
    K 折模型集成 + TTA 评估。
    对每个折的最优权重独立进行 TTA 推理，对 softmax 概率跨折取均值后做分类决策。
    相当于 K × n_tta 次独立预测的集成，最大程度降低方差。
    """
    ...
    for task in TASK_HEADS:
        if not fold_probs[task]:
            continue
        avg_probs   = np.mean(fold_probs[task], axis=0)   # (N, n_cls) 跨折均值
...
# 与 imagebindtrain/cedtrain 可视化对齐（图例、刻度、y 留白、混淆矩阵标题层级）
_TRAIN_LEGEND_FS = 22
_TRAIN_TICK_FS = 20
_TRAIN_AXIS_FS = 20
_TASK_LEGEND_LOC = {
...
def train_fold(
    ...
    """
    训练单折，返回 (best_state, history)。
    内部：WeightedRandomSampler 做逆频均衡；Focal 不带 class weight；四任务等权。
    """
    ...
    # 与 dinov3 一致：不再传 class_weights；采样器已均衡 Batch，Loss 叠加逆频会过补偿
    ...
    logger.info(f"  [折{fold_idx+1}] 任务损失权重（等权）: " +
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("大创项目 多分支路由 MLP 正式训练（学术级）")
    logger.info("=" * 60)
    log_environment()
    ...
    # ── 4. 绘制训练曲线（最后一折）─────────────────────────────
    logger.info("\n[4/5] 绘制训练曲线...")
    fig_path = FIG_DIR / f"train_curve_{TS}.png"
    ...
    if cm_data:
        from imagebindtrain.visualization import plot_confusion_matrices_split

        cm_img = FIG_DIR / f"confusion_split_image_{TS}.png"
        cm_aud = FIG_DIR / f"confusion_split_audio_{TS}.png"
        plot_confusion_matrices_split(cm_data, cm_img, cm_aud)
        logger.info("混淆矩阵（按模态拆分）已保存：%s ； %s", cm_img, cm_aud)
        try:
            from imagebindtrain.mlp_interpretability import run_mlp_interpretability_suite
    ...
    # ── 保存模型（含全部折权重，支持后续集成推理）───────────────
    model_path = MODEL_DIR / f"MultiBranchMLP_{TS}.pkl"
    torch.save({
        "model_states": fold_states,
        "n_folds":      N_FOLDS,
        "hyperparams": {
            "hidden_dim": HIDDEN_DIM, "input_dim": 1024,
            ...
            "species_loss_coef": 0.3, "task_loss_equal": True,
            "n_folds": N_FOLDS,
